...
2. 长距离来源引用优先用 Shadow Variable；但 Shadow 变量只能作为来源发出箭头，不能作为目标接收入箭头。目标变量应在所属子系统 View 中用 Defined 变量呈现。
3. 一个变量最多保留 3-5 条直接可见箭头；超过后拆 View 或把常量参数移入参数表。
4. 主路径从左到右，反馈路径从右下绕回左上。
5. 信息箭头交叉超过 3 条时必须拆子系统，不继续在一张图里堆线。
6. 箭头不能压在变量文字上；必要时增加控制点或移动控制点绕开文字。
7. 变量名不要过长；长变量名放论文变量表解释，图中用精炼中文名。
8. 不要把所有参数画出来，常量参数默认放“模型参数表”。
9. 截图前统一字体、字号、线宽和缩放比例。
10. 最终以“能解释模型机制”为准，不以“把所有变量画出来”为准。
11. Python 图表图例采用动态策略：1-2 条曲线可放图内空白角落；3-6 条曲线放右侧并压缩主图比例；超过 6 条曲线放底部多列或拆成多张图。不要一律外置，避免主图留白过大。
...
- **灰色尖括号 causes**：Defined 变量若方程引用了当前 View 未画出的原因变量，Vensim 会在变量下方显示灰色 `<原因变量>`。这通常不是仿真错误，而是视图没有把方程 causes 展开。处理方式是：在该子系统 View 画出必要流率/辅助变量，长距离来源用 Shadow Variable，或把常量参数移到参数表。
