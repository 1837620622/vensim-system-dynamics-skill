...
- `tools/academic_gate.py`：研究问题/边界、参考文献、存量初值、单位、历史内生性和耦合协调输出门禁；
...
中文项目默认使用中文业务变量。Vensim 控制变量和函数名保持原生英文，业务变量、图题、坐标轴、图例、CSV 表头和说明文档使用中文。工具已支持中文变量解析、仿真、审计和绘图。

论文级 Vensim 图默认遵守以下原则：

- 同一模块变量放近，避免跨半页连线；
- 长距离关系优先拆 View；确需 Shadow Variable 时只能发出箭头，主图必须在模型级设置中隐藏灰色 `<…>` 影子提示；
- 一个变量直接可见箭头控制在 3-5 条，过多就拆 View；
- 主路径左到右，反馈路径从右下绕回左上；
- 交叉线超过 3 条就拆子系统或建立反馈/结果辅助 View；
- 普通信息箭头必须是 `shape=1` + 至少一个控制点的深蓝实线，存量—流率实体管道保持黑色；
- GDP、开行、TEU、货值、网络和运输时间等带单位指标做归一化时，期初/期末锚点必须定义为同单位模型参数，禁止“带单位变量−裸数字”；
- 变量框不得重叠，直接可见箭头一般不超过 5 条；
- 箭头不压变量文字，变量名用精炼中文；
- 常量参数放参数表，不强行全画到图里；
- 截图前统一字体、字号、线宽和缩放；
- Python 图表图例采用动态策略：少量曲线可放图内空白角落，中等数量外置，很多曲线放底部多列或拆图，避免遮挡和过度留白；
- 图的目标是解释模型机制，不是把所有变量画出来。

`audit` 会额外提示长变量名、长距离信息箭头、节点箭头过多、变量框重叠和交叉线过多。灰色 `<变量名>` 是 Vensim 对 Defined 变量未展开 causes 的提示，不一定是方程错误，但论文主图不能留下；应补充子系统/反馈 View、使用辅助 View 和参数表，并写入 `27:64` 后在全新 Vensim 进程复核。

研究设计硬规则：先读领域文献、系统动力学方法文献和数据来源，再确定边界、变量、反馈和参数；历史验证必须由真实初值和同一套内生方程生成，观测序列只作外生边界或外部比较；若有耦合协调，U1/U2/C/T/D 必须由模型内生变量连续计算，历史熵权D不能作为输入路径。D、相对发展度等有意报告型终端输出的 USE FLAG 要在审计记录中解释，不得用零系数伪造引用。运行 `./skill.sh academic model.mdl --references <文献目录> --spec <model_spec.json>` 做门禁。
