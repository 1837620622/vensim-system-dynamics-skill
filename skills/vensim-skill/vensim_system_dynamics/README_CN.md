...
- 同一模块变量放近，避免跨半页连线；
- 长距离来源引用用 Shadow Variable，但 Shadow 只能发出箭头，不能作为结果接收入箭头；
...
`audit` 会额外提示长变量名、长距离信息箭头、节点箭头过多和交叉线过多。灰色 `<变量名>` 是 Vensim 对 Defined 变量未展开 causes 的提示，不一定是方程错误；应通过补充子系统 View、使用 Shadow 来源变量或参数表来处理。
