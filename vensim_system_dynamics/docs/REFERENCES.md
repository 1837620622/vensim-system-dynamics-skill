...
- 箭头字段：`1,id,from,to,shape,...,np|(x,y)|...`；`from,to` 为对象 ID；`np` 为控制点个数；`|(x,y)|` 为控制点列表。
...
## 3. PySD（辅助验证，不替代 Vensim）
https://pysd.readthedocs.io/en/master/structure/vensim_translation.html
- PySD 用 PEG 语法(`sketch.peg`)解析 `.mdl` 草图，可用于辅助校验解析结果。源码：https://github.com/SDXorg/pysd
...
- 物理流率管道(weight≥20 或端点为阀门/云)保持原样。
